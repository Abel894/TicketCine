// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Npgsql.EntityFrameworkCore.PostgreSQL.Metadata;
using TicketCine.Infrastructure.Data;

#nullable disable

namespace TicketCine.Infrastructure.Migrations
{
    [DbContext(typeof(TicketCineDbContext))]
    [Migration("20260705150210_AgregarReservasYVentas")]
    partial class AgregarReservasYVentas
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 63);

            NpgsqlModelBuilderExtensions.UseIdentityByDefaultColumns(modelBuilder);

            modelBuilder.Entity("TicketCine.Domain.Entities.Asiento", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<int>("Columna")
                        .HasColumnType("integer");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<int>("Fila")
                        .HasColumnType("integer");

                    b.Property<Guid>("FuncionId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("FuncionId", "Fila", "Columna")
                        .HasDatabaseName("idx_asientos_funcion_fila_columna");

                    b.ToTable("asientos", (string)null);
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.AsientoReserva", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<Guid>("AsientoId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("ReservaId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("AsientoId");

                    b.HasIndex("ReservaId", "AsientoId")
                        .IsUnique()
                        .HasDatabaseName("idx_asientos_reserva_reserva_asiento_unique");

                    b.ToTable("asientos_reserva", (string)null);
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Funcion", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("Activo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<DateTime>("FechaHora")
                        .HasColumnType("timestamp without time zone");

                    b.Property<Guid>("PeliculaId")
                        .HasColumnType("uuid");

                    b.Property<decimal>("Precio")
                        .HasColumnType("numeric(10, 2)");

                    b.Property<Guid>("SalaId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("FechaHora")
                        .HasDatabaseName("idx_funciones_fechahora");

                    b.HasIndex("PeliculaId");

                    b.HasIndex("SalaId");

                    b.ToTable("funciones", (string)null);
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Pelicula", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("Activo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("Clasificacion")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.Property<int>("DuracionMinutos")
                        .HasColumnType("integer");

                    b.Property<string>("Genero")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.Property<string>("RutaPoster")
                        .HasMaxLength(500)
                        .HasColumnType("character varying(500)");

                    b.Property<string>("Sinopsis")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<string>("Titulo")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.HasKey("Id");

                    b.ToTable("peliculas", (string)null);
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Reserva", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("Estado")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<DateTime>("FechaCreacion")
                        .HasColumnType("timestamp without time zone");

                    b.Property<DateTime>("FechaExpiracion")
                        .HasColumnType("timestamp without time zone");

                    b.Property<Guid>("FuncionId")
                        .HasColumnType("uuid");

                    b.Property<Guid>("UsuarioId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("FuncionId");

                    b.HasIndex("UsuarioId", "Estado")
                        .HasDatabaseName("idx_reservas_usuario_estado");

                    b.ToTable("reservas", (string)null);
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Rol", b =>
                {
                    b.Property<int>("Id")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("character varying(50)");

                    b.HasKey("Id");

                    b.ToTable("roles", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Nombre = "Cliente"
                        },
                        new
                        {
                            Id = 2,
                            Nombre = "Administrador"
                        });
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Sala", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("Activo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<int>("Columnas")
                        .HasColumnType("integer");

                    b.Property<int>("Filas")
                        .HasColumnType("integer");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("character varying(100)");

                    b.HasKey("Id");

                    b.ToTable("salas", (string)null);
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Usuario", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<bool>("Activo")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("boolean")
                        .HasDefaultValue(true);

                    b.Property<string>("Correo")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<DateTime>("FechaCreacion")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("timestamp with time zone")
                        .HasDefaultValueSql("CURRENT_TIMESTAMP");

                    b.Property<string>("Nombre")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("character varying(255)");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("text");

                    b.Property<int>("RolId")
                        .HasColumnType("integer");

                    b.HasKey("Id");

                    b.HasIndex("Correo")
                        .IsUnique()
                        .HasDatabaseName("idx_usuarios_correo_unique");

                    b.HasIndex("RolId");

                    b.ToTable("usuarios", (string)null);
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Venta", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uuid");

                    b.Property<string>("CodigoQr")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("character varying(250)");

                    b.Property<DateTime>("FechaVenta")
                        .HasColumnType("timestamp without time zone");

                    b.Property<string>("MetodoPago")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("character varying(20)");

                    b.Property<decimal>("MontoTotal")
                        .HasColumnType("numeric(10, 2)");

                    b.Property<Guid>("ReservaId")
                        .HasColumnType("uuid");

                    b.HasKey("Id");

                    b.HasIndex("ReservaId")
                        .IsUnique()
                        .HasDatabaseName("idx_ventas_reserva_id_unique");

                    b.ToTable("ventas", (string)null);
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Asiento", b =>
                {
                    b.HasOne("TicketCine.Domain.Entities.Funcion", "Funcion")
                        .WithMany("Asientos")
                        .HasForeignKey("FuncionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_asientos_funcion_id");

                    b.Navigation("Funcion");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.AsientoReserva", b =>
                {
                    b.HasOne("TicketCine.Domain.Entities.Asiento", "Asiento")
                        .WithMany()
                        .HasForeignKey("AsientoId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_asientos_reserva_asiento_id");

                    b.HasOne("TicketCine.Domain.Entities.Reserva", "Reserva")
                        .WithMany("Asientos")
                        .HasForeignKey("ReservaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_asientos_reserva_reserva_id");

                    b.Navigation("Asiento");

                    b.Navigation("Reserva");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Funcion", b =>
                {
                    b.HasOne("TicketCine.Domain.Entities.Pelicula", "Pelicula")
                        .WithMany("Funciones")
                        .HasForeignKey("PeliculaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_funciones_pelicula_id");

                    b.HasOne("TicketCine.Domain.Entities.Sala", "Sala")
                        .WithMany("Funciones")
                        .HasForeignKey("SalaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_funciones_sala_id");

                    b.Navigation("Pelicula");

                    b.Navigation("Sala");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Reserva", b =>
                {
                    b.HasOne("TicketCine.Domain.Entities.Funcion", "Funcion")
                        .WithMany()
                        .HasForeignKey("FuncionId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_reservas_funcion_id");

                    b.HasOne("TicketCine.Domain.Entities.Usuario", "Usuario")
                        .WithMany()
                        .HasForeignKey("UsuarioId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_reservas_usuario_id");

                    b.Navigation("Funcion");

                    b.Navigation("Usuario");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Usuario", b =>
                {
                    b.HasOne("TicketCine.Domain.Entities.Rol", "Rol")
                        .WithMany("Usuarios")
                        .HasForeignKey("RolId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_usuarios_rol_id");

                    b.Navigation("Rol");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Venta", b =>
                {
                    b.HasOne("TicketCine.Domain.Entities.Reserva", "Reserva")
                        .WithOne()
                        .HasForeignKey("TicketCine.Domain.Entities.Venta", "ReservaId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_ventas_reserva_id");

                    b.Navigation("Reserva");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Funcion", b =>
                {
                    b.Navigation("Asientos");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Pelicula", b =>
                {
                    b.Navigation("Funciones");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Reserva", b =>
                {
                    b.Navigation("Asientos");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Rol", b =>
                {
                    b.Navigation("Usuarios");
                });

            modelBuilder.Entity("TicketCine.Domain.Entities.Sala", b =>
                {
                    b.Navigation("Funciones");
                });
#pragma warning restore 612, 618
        }
    }
}
